...
def main_kinetics400(output_path=''):
    print('save to %s ... ' % output_path)
    # for splitname in ['val_split', 'train_split']:
    for splitname in ['train']:
        # v_root = '/datasets/KineticsVideo' + '/' + splitname
        v_root = '/apdcephfs/share_1290939/0_public_datasets/k400/data/videos' + '/' + splitname
        ...
        for i, j in tqdm(enumerate(v_act_root), total=len(v_act_root)):
            ...
            Parallel(n_jobs=1)(delayed(resize_video_ffmpeg)(p, out_path, dim=256) for p in tqdm(v_paths, total=len(v_paths)))



if __name__ == '__main__':
    main_kinetics400(output_path='/apdcephfs/share_1290939/qiangqwu/kinetics400')
